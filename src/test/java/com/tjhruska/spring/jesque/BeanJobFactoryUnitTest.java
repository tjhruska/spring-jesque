...
import com.tjhruska.spring.jesque.BeanJobFactory;
...
public class BeanJobFactoryUnitTest {
  ...
  public void wrongClassName() {
    ...
    try {
      ...
      assertEquals("Wrong exception message", 
          "BeanJobFactory only knows how to materialize BeanJob bean jobs.  " +
  ...
  public void initFoundBeanNonInitWithArgs() throws Exception {
    try {
      ...
      assertEquals("Wrong exception message", "Variable arguments passed into BeanJob required bean implement RunnableWithInit interface, bean testBeanJob doesn't.", e.getMessage());
    }
  }
  ...
  public void runFoundBeanPrototype() throws Exception {
    ...
    runner.run();
    assertTrue("Wrong run count, same bean instance should have incremented to 2", ((TestBeanJob)runner).runCount.equals(2));
  ...
  public void runFoundBeanInitializingPrototype() throws Exception {
    ...
    runner.run();
    assertTrue("Wrong run count, same bean should have been incremented to 2", ((TestInitializingBeanJob)runner).runCount.equals(2));
    
    runner = (Runnable) beanJobFactory.materializeJob(testBeanJob);
    runner.run();
    assertTrue("Wrong run count, prototype bean should have been same", ((TestInitializingBeanJob)runner).runCount.equals(1));
  }
  ...
  public void runFoundBeanSingleton() throws Exception {
    ...
    runner.run();
    assertTrue("Wrong run count, same bean should have been incremented to 2", ((TestBeanJob)runner).runCount.equals(2));
    
    runner = (Runnable) beanJobFactory.materializeJob(testBeanJob);
    runner.run();
    assertTrue("Wrong run count, singleton bean should have been incremented to 3", ((TestBeanJob)runner).runCount.equals(3));
  }
} 